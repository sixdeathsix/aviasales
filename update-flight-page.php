...
<div class="container df jcsb fw">
    ...
    <main class="df fc jcc aic">
        <form class="auth-form w-full df fc jcc aic" action="" method='post'>
            ...
            <input id="from" class='form-action' type="datetime-local" name="from_date"
                   value="<?= $flight['scheduled_departure'] ?>" required>
            <span>Время прибытия</span>
            <input id="to" class='form-action' type="datetime-local" name="to_date"
                   value="<?= $flight['scheduled_arrival'] ?>"
                   required>

            <input class='form-action' placeholder='Цена' value="<?= $flight['price'] ?>" name='price' type="number"
                   min='1' required>
